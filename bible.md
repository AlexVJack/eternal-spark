# Eternal Spark — Series Bible

Living document. Updated after every scene.
...
- Former division interface: smoother, haptic response
...
- Classified by that civilization as: speculative mathematics
- Flagged by intake analyst: *moderate value, long-cycle asset, recommend standard distribution*
- Core principle: the point where a system begins to improve its own architecture without external input
- Distribution queue: 47 registered buyers tomorrow, ~300 within a month
...
See `naming.md` for full rules.

- Structure: Given name · Clan · Region
- Megil = Memory + Guilt
- Dys = Disclosure
- Sin = Synthesis
...
See `scene-01-symbols.md` for the draws that built Scene 01.
Each scene will have its own symbols file.
